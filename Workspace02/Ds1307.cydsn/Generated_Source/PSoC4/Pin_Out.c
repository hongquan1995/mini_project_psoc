...
#include "Pin_Out.h"
    ...
    #define Pin_Out_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_Out_PC =   (Pin_Out_PC & \
                                (uint32)(~(uint32)(Pin_Out_DRIVE_MODE_IND_MASK << \
                                (Pin_Out_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_Out_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_Out_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_Out_USBIO_CTRL_REG = (Pin_Out_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_Out_DRIVE_MODE_IND_MASK << \
                                    (Pin_Out_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_Out_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_Out_SetP4PinDriveMode(Pin_Out__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Pin_Out_DR & (uint8)(~Pin_Out_MASK));
    drVal = (drVal | ((uint8)(value << Pin_Out_SHIFT) & Pin_Out_MASK));
    Pin_Out_DR = (uint32)drVal;
}
...
{
    return (uint8)((Pin_Out_PS & Pin_Out_MASK) >> Pin_Out_SHIFT);
}
...
{
    return (uint8)((Pin_Out_DR & Pin_Out_MASK) >> Pin_Out_SHIFT);
}
...
void Pin_Out_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_Out_INTCFG & (uint32)(~(uint32)position);
    Pin_Out_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Pin_Out_INTSTAT & Pin_Out_MASK);
	Pin_Out_INTSTAT = maskedStatus;
    return maskedStatus >> Pin_Out_SHIFT;
}
