...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_rx__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_rx__INTC_NUMBER];
}
...
void isr_rx_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_rx__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_rx_INTC_PRIOR = (*isr_rx_INTC_PRIOR & (uint32)(~isr_rx__INTC_PRIOR_MASK)) |
...
uint8 isr_rx_GetPriority(void)
{
    ...
    priority = (*isr_rx_INTC_PRIOR & isr_rx__INTC_PRIOR_MASK) >> priorityOffset;
