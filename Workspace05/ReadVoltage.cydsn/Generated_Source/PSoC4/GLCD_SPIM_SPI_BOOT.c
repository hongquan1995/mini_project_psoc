...
/*******************************************************************************
* Function Name: GLCD_SPIM_SpiCyBtldrCommStart
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: GLCD_SPIM_SpiCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: GLCD_SPIM_SpiCyBtldrCommReset
****************************************************************************//**
*
...
void GLCD_SPIM_SpiCyBtldrCommReset(void)
{
    ...
    GLCD_SPIM_SpiUartClearRxBuffer();
    GLCD_SPIM_SpiUartClearTxBuffer();
}


/*******************************************************************************
* Function Name: GLCD_SPIM_SpiCyBtldrCommRead
****************************************************************************//**
*
...
cystatus GLCD_SPIM_SpiCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (0u != GLCD_SPIM_SpiUartGetRxBufferSize())
            {
                /* Wait for packet end */
                do
                {
                    byteCount = GLCD_SPIM_SpiUartGetRxBufferSize();
                    CyDelayUs(GLCD_SPIM_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != GLCD_SPIM_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                GLCD_SPIM_RX_FIFO_CTRL_REG |= GLCD_SPIM_RX_FIFO_CTRL_FREEZE;

                byteCount = GLCD_SPIM_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) GLCD_SPIM_SpiUartReadRxData();
        }
    }
...
/*******************************************************************************
* Function Name: GLCD_SPIM_SpiCyBtldrCommWrite
****************************************************************************//**
*
*  Allows the caller to read data from the bootloader host (the host writes the
*  data). The function handles polling to allow a block of data to be completely
...
cystatus GLCD_SPIM_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        GLCD_SPIM_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == GLCD_SPIM_SpiUartGetTxBufferSize()) &&
                (0u == (GLCD_SPIM_SPI_STATUS_REG & GLCD_SPIM_SPI_STATUS_BUS_BUSY)) &&
                (0u == (GLCD_SPIM_TX_FIFO_STATUS_REG & GLCD_SPIM_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        GLCD_SPIM_SpiUartClearRxBuffer();
        GLCD_SPIM_RX_FIFO_CTRL_REG &= (uint32) ~GLCD_SPIM_RX_FIFO_CTRL_FREEZE;
    }
